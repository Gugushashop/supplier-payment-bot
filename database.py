...
def init_db():
    ...
    try:
        ...
        # Добавляем поставщиков из config.py
        for supplier_id, supplier_data in SUPPLIERS.items():
            cursor.execute('''
            INSERT OR IGNORE INTO suppliers (id, name, chat_id)
            VALUES (?, ?, ?)
            ''', (supplier_id, supplier_data.get('name'), supplier_data.get('chat_id')))
        
        conn.commit()
        conn.close()
        logger.info("✅ БД инициализирована успешно + добавлены поставщики")
